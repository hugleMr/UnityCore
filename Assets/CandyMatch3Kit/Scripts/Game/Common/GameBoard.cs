...
namespace GameVanilla.Game.Common
{
    ...
    public class GameBoard : MonoBehaviour
    {
        ...
        private void
        ExplodeTileRecursive(GameObject tile, List<GameObject> explodedTiles)
        {
        ...
        private bool HandleMatches(bool isPlayerMatch)
        {
            ...
            if (matches.Count > 0)
            {
                ...
                foreach (var match in matches)
                {
                    ...
                    Debug.Log("didAnySpecialCandyExplode" + didAnySpecialCandyExplode);
                        ...
                        else if (match.tiles.Count >= 4)
                        {
                            if (swapDirection == SwapDirection.Horizontal)
                            {
                                if (isPlayerMatch)
                                {
                                    if (match.tiles.Contains(lastSelectedTile))
                                    {
                                        Debug.Log("CreateHorizontalStripedTile");
                            ...
                            else
                            {
                                if (isPlayerMatch)
                                {
                                    if (match.tiles.Contains(lastSelectedTile))
                                    {
                                        Debug.Log("CreateVerticalStripedTile");
        ...
        private IEnumerator HighlightRandomMatchAsync()
        {
            ...
            Debug.Log("HighlightRandomMatch");
